...
- [ ] 2.1 Update `UserProfile` in `js/types.ts` to include `tracked_vintage_years: number[]`.
- [ ] 2.2 Replace `count2016`/`count2018`/`count2023` in the `Stats` interface (`js/types.ts`) with `trackedYearCounts: number[]`.
- [ ] 2.3 Update `computeStats()` in `js/utils.ts` to accept `trackedYears: number[]` and return `trackedYearCounts` computed over that input in the same order, instead of the three hardcoded vintage filters — verify: existing behavior for 2016/2018/2023 is unchanged when those are the input years.
